...
class LibJSAgent extends ConsoleAgent {
  constructor(options) {
    ...
    this.args.unshift('--disable-ansi-colors', '--no-syntax-highlight', '--disable-source-location-hints', '--disable-debug-output', '--use-test262-global');

    if (options.experimental) {
      // use bytecode
      // this.args.unshift('--run-bytecode');
    } else {
      // use ast
      this.args.unshift('--ast');
    }
  }
